...
public class LogMessageDetailBean extends LogMessageBean {
	...
	public LogMessageDetailBean(LogMessage logMessage) {
		super(logMessage);
		...
		try {
			...
			for (int i = 0; i < message.getAttachment().getUnnamedCount(); i++) {
				...
				attachments.put("unnamed-" + i, new NameValuePair("unnamed-"
						+ i, value));
			}
		...
		if (logMessage.getTags() != null) {
			for (Map.Entry<String, LogTag> entry : logMessage.getTags()
					.entrySet()) {
				tags.put(entry.getKey(), new NameValuePair(entry.getKey(),
						entry.getValue().getTagValue()));
			}
		}
