...
public class LogStoreService implements LogStoreServiceMBean
{
  ...
  private void dumpLogMessage(final Element parent, final LogMessage logMessage)
  {
    final Element logMessageElement = parent.addElement("log-message");
    logMessageElement.addAttribute("service-category", logMessage.getServiceCategory());
    logMessageElement.addAttribute("service-name", logMessage.getServiceName());
    ...
    logMessageElement.addAttribute("correlation-id", logMessage.getCorrelationId());
    if (logMessage.getJbpmProcessInstanceId() != null) {
      logMessageElement.addAttribute("jbpm-process-instance-id", String.valueOf(logMessage.getJbpmProcessInstanceId()));
    }
    if (logMessage.getJbpmNodeId() != null) {
      logMessageElement.addAttribute("jbpm-node-id", String.valueOf(logMessage.getJbpmNodeId()));
    }
    if (logMessage.getJbpmTokenId() != null) {
      logMessageElement.addAttribute("jbpm-token-id", String.valueOf(logMessage.getJbpmTokenId()));
    }
    if (logMessage.getFaultReason() != null) {
      final Element faultElement = logMessageElement.addElement("fault");
      faultElement.addElement("reason").addText(logMessage.getFaultReason());
      ...
      if (logMessage.getFaultCause() != null) {
        ...
        faultElement.addElement("cause").addCDATA(faultCause.toString());
      }
    }
    try {
      ...
      final Element messageElement = logMessageElement.addElement("message");

      final Element propertiesElement = messageElement.addElement("properties");
      for (final String name : message.getProperties().getNames()) {
        final Object value = message.getProperties().getProperty(name);
        final Element propertyElement = propertiesElement.addElement("property");
      ...
      for (final String name : message.getBody().getNames()) {
        ...
        if (value != null) {
          if (value instanceof NormalizedData) {
            final Element dataElement = bodyElement.addElement("normalized-data");

            ((NormalizedData) value).toXML(dataElement);
          } else if (value instanceof String && ((String) value).startsWith("<?xml")) {
            try {
              final SAXReader saxReader = new SAXReader();
              final Document dataDocument = saxReader.read(new StringReader((String) value));

              bodyElement.add(dataDocument.getRootElement().createCopy());
            } catch (final Exception e) {
              bodyElement.addCDATA(value.toString());
            }
          } else if (value instanceof byte[]) {
            bodyElement.addCDATA(new String((byte[]) value, "UTF-8"));
          } else {
            bodyElement.addCDATA(value.toString());
          }
        }
      ...
      final Element attachmentsElement = messageElement.addElement("attachments");
      for (final String name : message.getAttachment().getNames()) {
        final Object value = message.getAttachment().get(name);
        final Element attachmentElement = attachmentsElement.addElement("attachment");
        attachmentElement.addAttribute("name", name);
        if (value != null) {
          if (value instanceof byte[]) {
            attachmentElement.addCDATA(new String((byte[]) value, "UTF-8"));
          } else {
  ...
  private void dumpLogMessage(final PrintWriter out, final LogMessage logMessage)
  {
    out.println("-------");
    ...
    if (logMessage.getTags() != null) {
      out.println("Tags:");
      for (final Map.Entry<String, LogTag> entry : logMessage.getTags().entrySet()) {
        out.print("  ");
  ...
  public String showMessagesByProcessInstanceId(final long processInstanceId)
  {
    ...
    try {
      ...
      final Criteria criteria = session.createCriteria(LogMessage.class);
      criteria.add(Restrictions.eq("jbpmProcessInstanceId", processInstanceId));
  ...
  public String showMessagesByProcessInstanceIdAsHtml(final long processInstanceId)
  {
    String retVal = null;
    ...
    try {
      ...
      final Document logMessages = DocumentHelper.parseText(logMessagesXml);

      final ByteArrayOutputStream bos = new ByteArrayOutputStream();

      this.transformer.transform(new DocumentSource(logMessages), new StreamResult(bos));
  ...
  public String showMessagesByProcessInstanceIdAsXml(final long processInstanceId)
  {
  ...
  public String showMessagesByTag(final String tag, final String value, final long position)
  {
    final EncodedStringWriter writer = new EncodedStringWriter();
    final PrintWriter out = new PrintWriter(writer);
    Session session = null;
    ...
    try {
      ...
      criteria.add(Restrictions.and(Restrictions.and(Restrictions.eq("name", tag), Restrictions.eq("tagValue", value)),
  ...
  public String showMessagesByTagXML(final String tag, final String value, final long position)
  {
    final StringWriter writer = new StringWriter();
    Session session = null;
  ...
  public void start() throws Exception
  {
    ...
    final Dialect dialect = DialectFactory.determineDialect(vendor, majorVersion);
    ...
    cfg.setProperty(Environment.SESSION_FACTORY_NAME, LOG_SESSIONFACTORY_JNDI);
    cfg.setProperty(Environment.DATASOURCE, this.dataSourceJndiName);
    cfg.setProperty(Environment.TRANSACTION_MANAGER_STRATEGY, JBossTransactionManagerLookup.class.getName());
    ...
    this.transformer = factory.newTransformer(new StreamSource(LogStoreService.class
